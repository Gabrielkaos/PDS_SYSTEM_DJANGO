...
def bulk_export_forms(request):
    form_ids = request.GET.getlist('form_ids')
    
    if not form_ids:
        messages.warning(request, 'No forms selected for export')
        return redirect('home')
    
    
    forms = CompleteForm.objects.filter(id__in=form_ids, user=request.user)
    
    if not forms.exists():
        messages.error(request, 'No valid forms found')
        return redirect('home')
    ...
    for form in forms:
        ws = wb.create_sheet(title=f"{form.name[:25]}")  
        ...
        ws.cell(row=row, column=1, value="EDUCATIONAL BACKGROUND")
        row += 1
        e = form.educational_background
        ws.append(["Level", "Name of School", "Degree/Course", "From", "To", "Highest Level/Units", "Year Graduated", "Honors"])
        ws.append(["Elementary", e.elementary_name, e.elementary_degree_course, e.elementary_period_from, e.elementary_period_to, e.elementary_highest_level_units, e.elementary_year_graduated, e.elementary_honors])
        ...
        ws.cell(row=row, column=1, value="CIVIL SERVICE ELIGIBILITY")
        row += 1
        ws.append(["Career Service", "Rating", "Exam Date", "Exam Place", "License #", "License Validity"])
        for item in form.civil_service.all():
            ws.append([item.career_service, item.rating, item.exam_date, item.exam_place, item.license_number, item.license_validity])
        ...
        ws.cell(row=row, column=1, value="VOLUNTARY WORK")
        row += 1
        ws.append(["Organization", "From", "To", "Hours", "Nature"])
        for v in form.voluntary_work.all():
            ws.append([v.organization_name, v.from_date, v.to_date, v.number_of_hours, v.nature_of_work])
        ...
        ws.cell(row=row, column=1, value="LEARNING AND DEVELOPMENT")
        row += 1
        ws.append(["Title", "From", "To", "Hours", "Type", "Conducted By"])
        for l in form.learning_development.all():
            ws.append([l.title, l.from_date, l.to_date, l.number_of_hours, l.type_of_ld, l.conducted_by])
    ...
    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f'PDS_Bulk_Export_{len(forms)}_forms.xlsx'
    response['Content-Disposition'] = f'attachment; filename={filename}'
    wb.save(response)
            ...
            try:
                ...
                for sheet_name in wb.sheetnames:
                    ...
                    try:
                        ...
                        validator.validate_birth_date(data.get("Date of Birth"))
                        validator.validate_required_field(data.get("Place of Birth"), "Place of Birth")
                        validator.validate_sex(data.get("Sex"))
                        validator.validate_civil_status(data.get("Civil Status"))
                        validator.validate_height(data.get("Height (cm)"))
                        validator.validate_weight(data.get("Weight (kg)"))
...
def bulk_delete_forms(request):
    ...
    try:
        ...
        if not form_ids:
            return JsonResponse({'success': False, 'message': 'No forms selected'})
        
        
        deleted_count = CompleteForm.objects.filter(
            id__in=form_ids,
            user=request.user
        ).delete()[0]
        ...
        return JsonResponse({
            'success': True,
            'message': f'Successfully deleted {deleted_count} form(s)',
            'count': deleted_count
        })
    except Exception as e:
